...
Vue.component('store-sku-picker',{
    ...
    data: function(){
        return {
            skuPicker: skuPicker,
            colour: Object.keys(skuPicker[this.product])[0],
            size: Object.keys(skuPicker[this.product][Object.keys(skuPicker[this.product])[0]])[0]
        }
    },
    template: `
...
{{skuPicker}}{{product}}{{skuPicker[product]["red"]["petite"]}}
        <div class="btn-group" role="group">
        <button id="btnGroupDrop1" type="button" class="btn btn-secondary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        Size
        </button>
        <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
        <a v-for = "(item, key, index) in skuPicker[product][colour]" class="dropdown-item" href="#" @select="size = key">{{key}}</a>
        </div>
        </div>
        </div>
    `,
